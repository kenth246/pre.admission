...
export default function Admission() {
  ...
  const [applicants, setApplicants] = useState([
    {
      ...
      profile: {
        ...
        documents: [
          { name: "Birth Certificate", type: "IMG", format: "jpg", url: "https://tse2.mm.bing.net/th/id/OIP.TVUu-0ls3iKffitiH6_4dgHaKd?rs=1&pid=ImgDetMain&o=7&rm=3" }, 
          { name: "Report Card (Form 138)", type: "IMG", format: "png", url: "https://www.bing.com/th/id/OIP.rZgDhssot40e9DlWbHDoYQHaFt?w=193&h=149&c=8&rs=1&qlt=90&o=6&pid=3.1&rm=2" },
          { name: "Good Moral Certificate", type: "PDF", format: "pdf", url: "https://pdfobject.com/pdf/sample.pdf" }
        ]
      }
    },
    {
      ...
      location: "BALIUAG",
      date: "12/16/25",
      status: "Passed", // Changed from Failed to Passed
      assessment: {
        interviewScore: 92,
        interviewStatus: "Passed",
        bcetScore: "48/60", // Updated score to passing
        bcetStatus: "Passed" // Updated status
      },
      profile: {
        personal: {
          image: null,
          firstName: "ANNA", middleName: "M", surname: "SANTOS", suffix: "N/A",
          dob: "2005-08-20", pob: "BALIUAG", gender: "FEMALE", email: "anna@email.com",
          civilStatus: "SINGLE", contact: "09129998877", address: "789 RIZAL ST. BALIUAG, BULACAN"
        },
        family: {
          fatherName: "RICARDO SANTOS", fatherOcc: "CLERK", fatherContact: "09170001122",
          motherName: "LIZA SANTOS", motherOcc: "HOUSEWIFE", motherContact: "09173334455",
          guardianName: "N/A", guardianOcc: "N/A", guardianContact: "N/A",
          siblings: "1", income: "PHP 20,000 - 25,000"
        },
        education: {
          lrn: "404555666777",
          gwa: "86.00",
          elementary: "BALIUAG CENTRAL", elemAddr: "BALIUAG", elemType: "PUBLIC", elemYear: "2017",
          jhs: "BALIUAG HIGH", jhsAddr: "BALIUAG", jhsType: "PUBLIC", jhsYear: "2021",
          shs: "ST. MARYS COLLEGE", shsAddr: "BALIUAG", shsType: "PRIVATE", shsYear: "2023",
          lastSchool: "BULACAN STATE UNIVERSITY", 
          lastSchoolAddr: "MALOLOS", 
          lastSchoolType: "PUBLIC",
          lastSchoolYear: "2024"
        },
        documents: [
          { name: "Transcript of Records", type: "PDF", format: "pdf", url: "https://pdfobject.com/pdf/sample.pdf" },
          { name: "Honorable Dismissal", type: "PDF", format: "pdf", url: "https://pdfobject.com/pdf/sample.pdf" }
        ]
      }
    }
  ]);
  ...
  const handleAddSubmit = () => {
    ...
    // Determine status based on assessment inputs (simple logic: both must be passed)
  ...
  // --- PDF EXPORT FUNCTION (ALL) ---
  const handleExportPDF = () => {
    const doc = new jsPDF('landscape');
    doc.setFontSize(18);
    doc.setTextColor(0, 0, 0); 
    doc.text("Admission Applicants Report", 14, 20);
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0); 
    doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 26);
    const tableColumn = ["ID", "Name", "Type", "Location", "Contact", "Date", "Status", "Interview", "BCET"];
    const tableRows = filteredApplicants.map(app => [
        app.id,
        app.name,
        app.type,
        app.location,
        app.profile?.personal?.contact || "N/A",
        app.date,
        app.status,
        app.assessment?.interviewScore ? `${app.assessment.interviewScore} (${app.assessment.interviewStatus})` : 'N/A',
        app.assessment?.bcetScore ? `${app.assessment.bcetScore} (${app.assessment.bcetStatus})` : 'N/A',
    ]);
    autoTable(doc, {
        head: [tableColumn],
        body: tableRows,
        ...
        headStyles: { fillColor: [255, 255, 255], textColor: 0, fontStyle: 'bold', lineColor: [0, 0, 0] },
        alternateRowStyles: { fillColor: [245, 245, 245] },
    });
    doc.save(`Admission_Report_${new Date().toISOString().split('T')[0]}.pdf`);
  };

  // --- PDF PRINT FUNCTION (SINGLE) ---
  const handlePrintApplicant = () => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(0, 0, 0);
    doc.text("Applicant Profile", 105, 20, null, null, "center");
    ...
    const addSection = (title, data) => {
        autoTable(doc, {
            startY: finalY,
            head: [[{ content: title, colSpan: 2, styles: { halign: 'left', fillColor: [0, 0, 0], textColor: 255, fontStyle: 'bold' } }]],
            body: data,
            theme: 'grid',
            styles: { fontSize: 9, cellPadding: 2, lineColor: [0, 0, 0], lineWidth: 0.1, textColor: [0,0,0] },
            columnStyles: { 0: { fontStyle: 'bold', cellWidth: 50 } }, // Label column bold
        });
    ...
    // Personal Data
    const personalData = [
        ["Full Name", `${selectedApplicant.profile.personal.surname}, ${selectedApplicant.profile.personal.firstName} ${selectedApplicant.profile.personal.middleName}`],
        ["Applicant ID", selectedApplicant.id],
        ["Type", selectedApplicant.type],
        ["Date of Birth", selectedApplicant.profile.personal.dob],
        ["Gender", selectedApplicant.profile.personal.gender],
        ["Civil Status", selectedApplicant.profile.personal.civilStatus],
        ["Email", selectedApplicant.profile.personal.email],
        ["Contact", selectedApplicant.profile.personal.contact],
        ["Address", selectedApplicant.profile.personal.address],
    ];
    addSection("PERSONAL INFORMATION", personalData);

    // Family Data
    const familyData = [
        ["Father's Name", selectedApplicant.profile.family.fatherName],
        ["Mother's Name", selectedApplicant.profile.family.motherName],
        ["Guardian's Name", selectedApplicant.profile.family.guardianName],
        ["Siblings", selectedApplicant.profile.family.siblings],
        ["Family Income", selectedApplicant.profile.family.income],
    ];
    // Check page break
    if (finalY > 250) { doc.addPage(); finalY = 20; }
    addSection("FAMILY INFORMATION", familyData);

    // Education Data
    const educationData = [
        ["LRN", selectedApplicant.profile.education.lrn],
        ["GWA", selectedApplicant.profile.education.gwa],
        ["Elementary", `${selectedApplicant.profile.education.elementary} (${selectedApplicant.profile.education.elemYear})`],
        ["Junior High", `${selectedApplicant.profile.education.jhs} (${selectedApplicant.profile.education.jhsYear})`],
        ["Senior High", `${selectedApplicant.profile.education.shs} (${selectedApplicant.profile.education.shsYear})`],
    ];
    if (selectedApplicant.type === 'Transferees') {
        educationData.push(["Last School", `${selectedApplicant.profile.education.lastSchool}`]);
    }
    if (finalY > 250) { doc.addPage(); finalY = 20; }
    addSection("EDUCATION PROFILE", educationData);

    // Assessment Data
    const assessmentData = [
        ["Interview Status", selectedApplicant.assessment.interviewStatus],
        ["Interview Score", `${selectedApplicant.assessment.interviewScore}`],
        ["BCET Status", selectedApplicant.assessment.bcetStatus],
        ["BCET Score", `${selectedApplicant.assessment.bcetScore}`],
        ["Overall Status", selectedApplicant.status],
    ];
    if (finalY > 250) { doc.addPage(); finalY = 20; }
    addSection("ASSESSMENT RESULTS", assessmentData);

    // Save
    doc.save(`${selectedApplicant.id}_Profile.pdf`);
  };
            ...
            <div className="p-6 overflow-y-auto space-y-6 bg-gray-100">
              ...
              <div className="bg-white rounded-xl border border-gray-300 p-5 shadow-sm">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pb-2 border-b border-gray-100">
                    <div className="md:col-span-2">
                      <DataBox label="LRN/ Studnet Number" value={selectedApplicant.profile.education.lrn} />
                    </div>
                    <div className="md:col-span-2">
